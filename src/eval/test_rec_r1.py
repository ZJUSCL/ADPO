...
def main(args):
    ...
    model_path = args.model_path if args.steps == 0 else f"{args.model_path}/checkpoint-{args.steps}"
    ...
    for ds in test_datasets:
        ...
        # Process data
        for i in tqdm(range(0, len(messages), args.bsz), disable=rank != main_rank):
        ...
        if rank == main_rank:
            ...
            output_path = output_path_template.format(DATASET=ds, RUN_NAME=args.run_name, STEPS=args.steps)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Distributed evaluation of REC model (ADPO/R1 checkpoint).")
    parser.add_argument("--model_path", type=str, required=True,
                        help="Path to the model or checkpoint base directory.")
    parser.add_argument("--steps", type=int, default=0,
                        help="Checkpoint step number. If >0, loads <model_path>/checkpoint-<steps>. Default: 0 (load model_path directly).")
    parser.add_argument("--output_path", type=str,
                        default="./logs/rec_results_{DATASET}_{RUN_NAME}_{STEPS}.json",
                        help="Output path template. Supports {DATASET}, {RUN_NAME}, {STEPS} placeholders.")
    parser.add_argument("--data_root", type=str, required=True,
                        help="Root directory containing dataset JSON files.")
    parser.add_argument("--image_root", type=str, required=True,
                        help="Root directory containing images.")
    parser.add_argument("--test_datasets", type=str, default="refcoco_val,refcocop_val,refcocog_val",
                        help="Comma-separated list of dataset names to evaluate. Default: refcoco_val,refcocop_val,refcocog_val")
    parser.add_argument("--run_name", type=str, default="model",
                        help="Run name used in the output filename template.")
    parser.add_argument("--bsz", type=int, default=2,
                        help="Per-rank batch size. Default: 2")
    parser.add_argument("--num_samples", type=int, default=2000,
                        help="Max samples per dataset (shuffled with seed 42). 0 = use all. Default: 2000")
