...
from open_r1.trainer import ADPOConfig, VLMADPOTrainer
...
@dataclass
class ADPOScriptArguments(ScriptArguments):
    """
    Script arguments for the ADPO training script.
    """
...
@dataclass
class ADPOModelConfig(ModelConfig):
...
def main(script_args, training_args, model_args):
                ...
                'prompt': [{
                    ...
                    'content': [
                        *({'type': 'image', 'text': None} for _ in range(len(example['image_path']))),
                        {'type': 'text', 'text': question_prompt.format(Question=example['problem']) + "After outputting the answer, assign a score between 0-1 to this answer to express whether this answer provides accurate bounding box coordinates that correctly satisfy the user query. For example: <score>0.5</score>"}
                    ]
                }]
            }
        else:
    ...
    # Use the ADPO trainer entrypoint
    trainer_cls = VLMADPOTrainer
...
if __name__ == "__main__":
    parser = TrlParser((ADPOScriptArguments, ADPOConfig, ADPOModelConfig))
