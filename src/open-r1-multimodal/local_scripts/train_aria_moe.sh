...
# Example: export HF_TOKEN="your_actual_token"
if [ -z "${HF_TOKEN}" ]; then
    echo "Error: HF_TOKEN is not set. Please export it before running." >&2
    exit 1
fi
...
# Pick the first port from worker 0
...
# Example: export WANDB_API_KEY="your_actual_key"
if [ -z "${WANDB_API_KEY}" ]; then
    echo "Error: WANDB_API_KEY is not set. Please export it before running." >&2
    exit 1
fi
...
# Set PROJECT_ROOT to the path of this repository before running.
# Example: export PROJECT_ROOT="/path/to/multimodal-open-r1"
if [ -z "${PROJECT_ROOT}" ]; then
    echo "Error: PROJECT_ROOT is not set. Please export it before running." >&2
    exit 1
fi
cd "${PROJECT_ROOT}"
